...
class StatGhostConv(nn.Module):
    """针对统计特征的Ghost卷积"""
    def __init__(self, in_ch, out_ch, kernel_size=3, ratio=2):
        super().__init__()
        init_ch = out_ch // ratio
        self.primary = nn.Conv2d(in_ch, init_ch, kernel_size, 
                               padding=kernel_size//2, bias=False)
        self.cheap = nn.Sequential(
            nn.Conv2d(init_ch, init_ch, 3, padding=1, groups=init_ch, bias=False),
            nn.Conv2d(init_ch, out_ch - init_ch, 1, bias=False),
        )
...
class LiteChannelAttention(nn.Module):
    def __init__(self, in_ch, reduction=4, groups=4):
        super().__init__()
        self.groups = groups
        hidden_ch = max(in_ch // reduction // groups * groups, 4)  # 确保可分组
        
        self.ghost_attn = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_ch, hidden_ch, 1, groups=groups),  # 分组Pointwise
            nn.GELU(),
            nn.Conv2d(hidden_ch, in_ch, 1, groups=groups),  # 保持分组
            nn.Sigmoid()
        )

    def forward(self, x):
        return x * self.ghost_attn(x)
...
class LiteCBAM(nn.Module):
    def __init__(self, in_ch, reduction=4, groups=4):
        super().__init__()
        self.ca = LiteChannelAttention(in_ch, reduction, groups)
        self.sa = LiteSpatialAttention(kernel_size=3)
...
class MobileNetBlock(nn.Module):
    """
    MobileNet 模块，使用深度可分离卷积
    """
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, expansion=1):
        super(MobileNetBlock, self).__init__()
        hidden_dim = in_ch * expansion  # 扩展后的通道数
...
class GestureNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv2 = StatGhostConv(32, 64, kernel_size=3)
        self.bn2 = nn.BatchNorm2d(64)
        self.cbam2 = CBAM(64)

        self.conv3 = StatGhostConv(64, 128, kernel_size=3)
        self.bn3 = nn.BatchNorm2d(128)
        self.cbam3 = LiteCBAM(128)

        self.conv4 = StatGhostConv(128, 256, kernel_size=3)
        self.bn4 = nn.BatchNorm2d(256)
        self.cbam4 = LiteCBAM(256)
...
def train_gesture_model():
    ...
    # 数据路径
    data_path = r'E:\PROJECTS\testdata'
